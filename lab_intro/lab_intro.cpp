...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (int x = 0; x < image.width(); x++) {
    for (int y = 0; y < image.height(); y++) {
      HSLAPixel *pixel = image.getPixel(x,y);
      int xDiff = abs((int) (centerX - x));
      int yDiff = abs((int) (centerY - y));
      int euclideanDistance = round(sqrt(pow(xDiff, 2) + pow(yDiff, 2)));
      if (euclideanDistance > 200) {
        pixel->l = 0;
      } else {
        double divisor = 1 - (euclideanDistance * 0.005);
        pixel->l *= divisor;
      }
    }
  }
...
PNG ubcify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel *pixel = image.getPixel(x, y);
      double& hue = pixel->h;
      if (abs(ubcYellow - hue) < abs(ubcBlue - hue)) {
        pixel->h = ubcYellow;
      } else {
        pixel->h = ubcBlue;
      }
