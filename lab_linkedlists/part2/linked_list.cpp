...
Node* interleave(Node* list1, Node* list2){
  ...
  if (! list1 && ! list2) {   
    ...
    // empty list1   
    returnHead->key = list2->key;
    returnHead->next = interleave(NULL, list2->next);
  } else if (! list2) {
    // empty list2
    returnHead->key = list1->key;
    returnHead->next = interleave(list1->next, NULL);
  } else {
    returnHead->key = list1->key;
    // second node to insert
    Node* second = new Node();
    second->key = list2->key;
    returnHead->next = second;
    second->next = interleave(list1->next, list2->next);
  }
  return returnHead;
}
