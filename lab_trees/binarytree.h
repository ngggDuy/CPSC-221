...
class BinaryTree
{
  ...
  private:
     ...
     * @param subRoot The current node in the recursion.
     * @param path    Queue to hold current paths.
     */
    void printPaths(Node* subRoot, queue<Node*> paths) const;

    /**
     * Private helper function for the public printPaths function.
     * @param subRoot       The current node in the recursion.
     * @param currDistance  The current distance.
     */
    int sumDistances(Node* subRoot, int currDistance) const;
     ...
     */
    bool isOrdered(Node* subRoot, int max, int min) const;
